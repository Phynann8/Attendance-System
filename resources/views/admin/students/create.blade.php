...
@section('content')
<div class="page-title">{{ __('Add Student') }}</div>
<div class="page-sub">{{ __('Create a student record details.') }}</div>

<div class="card" style="max-width:720px; margin: auto;">
    <form method="POST" action="{{ route('admin.students.store') }}">
        @csrf
        <div class="form-group">
            <label for="name">{{ __('Student name') }} <span class="required">*</span></label>
            <input id="name" name="name" value="{{ old('name') }}" required>
            @error('name')<div class="error">{{ $message }}</div>@enderror
        </div>

        <div class="form-group">
            <label for="class_id">{{ __('Class') }} <span class="required">*</span></label>
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                <label for="parent_user_id">{{ __('Link parent account (optional)') }}</label>
        ...
        <a href="{{ route('admin.students.index') }}" class="btn btn-sm btn-outline">{{ __('Cancel') }}</a>
    </form>
</div>
@endsection